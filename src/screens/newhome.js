...
import {
  ...
  ActivityIndicator, RefreshControl, InteractionManager,
} from 'react-native';
...
const NotifBtn = memo(({ movieId }) => {
  ...
  return (
    ...
    >
      <LinearGradient colors={['rgba(0,255,178,0.15)','rgba(0,255,178,0.05)']} style={[StyleSheet.absoluteFill,{borderRadius:rs(16)}]} />
    ...
    <TouchableOpacity onPress={onPress} onPressIn={onIn} onPressOut={onOut} activeOpacity={1} style={style}>
      <Animated.View style={[S.glassBtn, accent && S.glassBtnAccent, { transform:[{scale:anim}] }]}>
        {accent ? (
          ...
          <LinearGradient colors={['rgba(255,255,255,0.12)','rgba(255,255,255,0.05)']} style={[StyleSheet.absoluteFill,{borderRadius:rs(28)}]} />
        )}
        ...
        <LinearGradient colors={['rgba(0,0,0,0)','rgba(0,0,0,0.14)']} start={{x:0,y:0.55}} end={{x:0,y:1}} style={[StyleSheet.absoluteFill,{borderRadius:rs(28)}]} />
  ...
  return (
    <Animated.View style={[S.header, { paddingTop: insets.top + rs(6), height: HEADER_H + insets.top, backgroundColor: bgColor }]}>
      ...
      <View style={S.headerRight}>
        <IconBtn icon="🔍" onPress={() => { console.log('Search'); /* TODO: navigate SearchScreen */ }} />
    ...
    <View style={[S.heroWrap, { height: HERO_H }]}>
      <Animated.View style={[StyleSheet.absoluteFill, { opacity:fadeA, transform:[{translateX:slideA}] }]}>
        <ImageBackground source={{ uri: item.hero_image || item.poster }} style={StyleSheet.absoluteFill} resizeMode="cover">
          <LinearGradient colors={['rgba(3,15,12,0.10)','rgba(3,15,12,0.0)','rgba(3,15,12,0.52)','rgba(3,15,12,1.0)']} locations={[0,0.20,0.60,1]} style={StyleSheet.absoluteFill} />
          <LinearGradient colors={['rgba(3,15,12,0.55)','rgba(3,15,12,0.0)']} start={{x:0,y:0.5}} end={{x:1,y:0.5}} style={StyleSheet.absoluteFill} />
          {/* Bottom blends into screen bg */}
          <LinearGradient colors={['rgba(3,15,12,0)', COLORS.bg]} start={{x:0.5,y:0.80}} end={{x:0.5,y:1}} style={StyleSheet.absoluteFill} />
        </ImageBackground>
      </Animated.View>
    ...
    <TouchableOpacity onPressIn={onIn} onPressOut={onOut} activeOpacity={1}
      ...
      <Animated.View style={[S.contCard, { transform:[{scale:anim}] }]}>
        ...
        <View style={S.contInner}>
          <ImageBackground source={{ uri: item.poster }} style={S.contImg} imageStyle={{ borderTopLeftRadius:rs(10), borderTopRightRadius:rs(10) }}>
            <LinearGradient colors={['rgba(3,15,12,0)','rgba(3,15,12,0.82)']} style={[StyleSheet.absoluteFill,{borderRadius:rs(10)}]} />
            {item.is_series && <SeriesBadge />}
            <View style={S.progOuter}>
              <View style={S.progBg} />
              <View style={[S.progFill, { width: `${Math.min(Math.round((item.progress||0)*100),100)}%` }]} />
            </View>
          </ImageBackground>
      ...
      <Animated.View style={[S.genreCard, { transform:[{scale:anim}] }]}>
        ...
        <View style={S.genreInner}>
          ...
          <View style={S.genreInfo}>
            <LinearGradient colors={['rgba(0,255,178,0.07)','rgba(3,15,12,0.5)']} style={[StyleSheet.absoluteFill,{borderBottomLeftRadius:rs(10),borderBottomRightRadius:rs(10)}]} />
            <Text style={S.genreTitle} numberOfLines={1}>{item.title}</Text>
          </View>
        </View>
      </Animated.View>
    </TouchableOpacity>
...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    StatusBar.setHidden(true, 'fade');
    startShimmer(); // start the one shared shimmer
    const t = setTimeout(() => {
      // Defer heavy data fetch until after first paint
...
const S = StyleSheet.create({
  ...
  header: {
    ...
    shadowColor: COLORS.accent, shadowOffset:{width:0,height:rs(3)}, shadowRadius:rs(12), elevation:14,
  },
  headerBorder: {
